...
class SubscriptionMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        # Проверка подписки имеет смысл только в личке пользователя. Раньше она
        # выполнялась и для сообщений в группе поддержки, включая топики
        # диалогов — если админ вдруг не подписан на required_channels, его
        # ответ пользователю блокировался этим мидлварём (в топик уходило
        # сообщение "подпишись", а сам ответ админа не пересылался), из-за чего
        # переписка "терялась" без какой-либо ошибки в логах.
        # ВАЖНО: у CallbackQuery нет своего .chat — чат нужно брать из
        # callback_query.message.chat (и он может отсутствовать, если исходное
        # сообщение было удалено/недоступно — тогда просто не блокируем).

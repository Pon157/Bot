...
from aiogram.types import BufferedInputFile, CallbackQuery, InlineKeyboardMarkup, Message, WebAppInfo
...
async def show_profile(message: Message, session: AsyncSession) -> None:
    ...
    # ссылка на веб-версию профиля (мини-приложение) — раньше нигде не была
    # доступна пользователю, хотя бэкенд (/api/profile) для неё уже был готов
    markup = None
    if settings.profile_webapp_url:
        markup = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    inline_btn(
                        "Открыть в приложении",
                        web_app=WebAppInfo(url=settings.profile_webapp_url),
                        emo=e.EYES,
                        style="primary",
                    )
                ]

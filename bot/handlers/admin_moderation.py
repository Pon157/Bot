...
# Порог предупреждений, после которого пользователь получает автобан.
MAX_ACTIVE_WARNS = 3


async def _count_active_warns(session: AsyncSession, user_id: int) -> int:
    result = await session.execute(
        select(Warn).where(Warn.user_id == user_id, Warn.active.is_(True))
    )
...
async def cmd_warn(message: Message, command: CommandObject, session: AsyncSession, db_admin: Admin | None) -> None:
    ...
    active_warns = await _count_active_warns(session, user.telegram_id)

    if active_warns >= MAX_ACTIVE_WARNS and not user.is_banned:
        user.is_banned = True
        user.ban_reason = f"Автобан: набрано {active_warns} предупреждения(-ий)"
        user.banned_at = datetime.now(timezone.utc)
        await session.commit()

        await message.reply(
            f"{ce(e.WARNING)} Пользователю выдано предупреждение ({active_warns}/{MAX_ACTIVE_WARNS}). "
            f"Причина: {reason}\n"
            f"{ce(e.NO_ENTRY)} Достигнут лимит предупреждений — пользователь автоматически забанен."
        )
        try:
            await message.bot.send_message(
                user.telegram_id,
                f"{ce(e.WARNING)} Тебе выдано предупреждение. Причина: {reason}\n\n"
                f"{ce(e.NO_ENTRY)} Это твоё {active_warns}-е предупреждение из {MAX_ACTIVE_WARNS} — "
                f"ты автоматически заблокирован(а) в боте.",
            )
        except Exception:
            pass
        return
    ...
    await message.reply(
        f"{ce(e.WARNING)} Пользователю выдано предупреждение ({active_warns}/{MAX_ACTIVE_WARNS}). Причина: {reason}"
    )
    try:
        await message.bot.send_message(
            user.telegram_id,
            f"{ce(e.WARNING)} Тебе выдано предупреждение ({active_warns}/{MAX_ACTIVE_WARNS}). Причина: {reason}",
        )
    except Exception:
        pass

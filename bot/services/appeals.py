...
async def accept_appeal(session: AsyncSession, bot: Bot, appeal: Appeal, admin: Admin) -> int:
    ...
    # удаляем сообщение о новом обращении в общем топике — оно больше не
    # актуально, обсуждение теперь идёт в отдельном топике обращения
    try:
        await bot.delete_message(settings.support_group_id, appeal.common_message_id)
    except Exception:
        pass
